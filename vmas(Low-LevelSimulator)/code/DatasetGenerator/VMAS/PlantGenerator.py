...
class PlantGenerator(VMASGenerator):
    ...
    class Expert():
        ...
        def initMap(self):
            boxes = []
            corners = []
            with open(os.path.dirname(os.path.abspath(__file__))+"/plans/plant_map.txt", "r") as f:
                text = f.read()
        ...
        #     plan = []
        #     with open(os.path.dirname(os.path.abspath(__file__))+"/plans/house.txt", "r") as f:
        #         text = f.read()
        ...
        def load_hl_plan(self):
                plan = []
                with open(os.path.dirname(os.path.abspath(__file__)) + "/plans/plant.txt", "r") as f:
                    text = f.read()

                for step_str in text.strip().split("\n"):
                    ...
                    for robot in step_str.split("-"):
                        robot = robot.strip()
                        if not robot:
                            continue  # Skip empty robot entries

                        try:
                            id_str = robot.split("(")[0][1:]  # Get the id part after 'r'
                            ...
                            print(f"Skipping malformed robot entry: '{robot}', error: {e}")
                            continue  # Skip malformed robot entries
        ...
        class PotentialFieldsAgent():
            ...
            def resetGoals (self):
                self.goals = torch.tensor(ROIs["y10"]).unsqueeze(0).repeat(self.task.numAgents, 1)
            ...
            def forward(self, pos):
                ...
                forces = 5.5 * attraction + 0.275*repulsion_agents + 0.01*repulsion_wall
